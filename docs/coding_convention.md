...
*   **過去形の命名と集約接頭辞**: 
    ...
    *   また、どの集約に関する事実かを明確にするため、必ず **集約名を接頭辞として付与** する（例: `TravelPlanned`, `FlightDelayed`, `TicketAnomalyDetected`）。
...
*   イベント（例: `TicketAnomalyDetected`）には、判断根拠となった外部情報の要約（`statusSummary`）を含める。
