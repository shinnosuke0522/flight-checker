...
#### 1.4 Sealed Class/Interface の定義
*   **メンバーのフラット化**: `sealed class` または `sealed interface` を継承するサブクラスは、基底クラスの内部にネストさせず、同一ファイル内のトップレベルで定義する。これは、可読性の向上と循環参照の防止、およびシリアライゼーションの容易性を目的とする。

#### 1.5 ExceptionではなくErrorベースのエラーハンドリング
...
*   **過去形の命名と集約接頭辞**: 
    *   イベントは「システム内で既に発生した事実」であるため、命名は必ず過去形とする。
    *   また、どの集約に関する事実かを明確にするため、必ず **集約名を接頭辞として付与** する（例: `TravelPlanned`, `FlightDelayed`, `TicketFlightCanceled`）。
...
#### 6.1 終端状態パターン (Terminal State Pattern)
一度到達すると二度と戻れない「終端状態」を持つ集約の場合、以下のガードを徹底する。
*   **モデル層 (apply)**: `apply` メソッド内で、現在の状態が終端状態（例: `FinishedTicket`）である場合、いかなるイベントが適用されても `this` を返し、状態の変化をロックする。
*   **サービス層**: ビジネスロジックを決定するドメインサービスの引数型を、可能な限り「非終端状態（例: `WatchingTicket` または `Ticket` でガード）」に制限し、終了済みのオブジェクトに対する操作をコンパイルレベルで排除する。

#### 6.2 ステータスメモリ・パターン (Status Memory Pattern)
監視システムなど、外部の事実と同期する集約において、無限通知の抑制や状況の変化を検知するために、「最後に承諾・検知した事実」を集約内に保持する。
*   イベント（例: `TicketFlightDelayed`）には、判断根拠となった外部情報の要約（`detail`）を含める。
*   集約は、現在の事実と自身のメモリ（記憶）を比較し、差分がある場合のみ新たなイベントを発行する。
...
*   **命名規則**: 基底となるインターフェースおよび、それを継承するすべてのサブクラス（具体的なコマンド）には、**必ず `Command` というサフィックスを付与**する（例: `TicketArrivedReflectCommand`）。これにより、そのクラスが「実行すべき意図」を表す入力であることを明確にする。
